...
## 文件描述
---
2025/5/13 於專案中開發自行上傳 Excel，導致部分 Table 資料錯誤，故申請回滾至 2025/5/12。  
...
![](aws-rds-table-restore-from-backup_images/image-1747212988238.png)  

**自訂日期和時間**：需大於最早可還原時間點，否則會通知錯誤無法建立還原  
![](aws-rds-table-restore-from-backup_images/image-1747213221729.png)  

**資料庫執行個體識別符**：自訂新的執行個體名稱  
![](aws-rds-table-restore-from-backup_images/image-1747213525058.png)  

**容量範圍**：可參考原執行個體做設定  
![](aws-rds-table-restore-from-backup_images/image-1747213659233.png)  
原執行個體設定   
![](aws-rds-table-restore-from-backup_images/image-1747213851523.png)

**還原至時間點**：其他設定維持預設值，下滑至頁面底部點選「還原至時間點」，接著系統會建立新執行個體  
![](aws-rds-table-restore-from-backup_images/image-1747214097162.png)  
...
![](aws-rds-table-restore-from-backup_images/image-1747279604129.png)
...
* RDS 快照  
  ...
  下滑至「快照」區塊，點選「建立快照」  
![](aws-rds-table-restore-from-backup_images/image-1747280464046.png)
  自訂快照名稱後，點選「建立快照」  
![](aws-rds-table-restore-from-backup_images/image-1747292475288.png)  
...
![](aws-rds-table-restore-from-backup_images/image-1747297709254.png)

當逐個 SQL 檔執行完成後，按下「F5」重新整理    
進入資料庫頁面，核對已還原的 Table 筆數是否正確  
![](aws-rds-table-restore-from-backup_images/image-1747291560238.png)  
...
修改 SQL 檔中的 Table 名稱  
![](aws-rds-table-restore-from-backup_images/image-1747385737054.png)  

#### 2. 執行 SQL 檔，並檢查新 Table 相關資訊（例如結構及資料筆數）
執行 SQL 檔建立新表  
![](aws-rds-table-restore-from-backup_images/image-1747388695603.png)  

檢查筆數  
![](aws-rds-table-restore-from-backup_images/image-1747387228840.png)  
...
![](aws-rds-table-restore-from-backup_images/image-1747387876494.png)  

<p class="callout info" id="mrk-5">若有觸發器 SQL 檔，則在 RENAME 後執行。因為既有的觸發器會跟著舊表名，執行觸發器 SQL 檔後會更新為正確表名。</p>

#### 4. 刪除舊 Table
![](aws-rds-table-restore-from-backup_images/image-1747388495900.png)  
